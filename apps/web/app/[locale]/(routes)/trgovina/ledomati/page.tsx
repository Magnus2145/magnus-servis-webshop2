...
export default async function LedomatiPage({
  params,
  searchParams,
}: {
  ...
  const category = await fetchCategoryByHandle('ledomati');
  const brand = searchParams.brand;
  const [products, page] = await Promise.all([
    category ? fetchProducts({ 'category_id[]': category.id, 'tags[]': brand }).catch(() => []) : Promise.resolve([]),
    fetchPageBySlug('trgovina-ledomati', params.locale),
  ]);
  ...
  return (
    <div className="space-y-12">
      <PageBuilder sections={page?.sections ?? []} locale={params.locale} />
        ...
        <section className="space-y-6">
          <header className="space-y-2">
            <h1 className="text-2xl font-semibold text-primary">
              {page?.title ?? translate('filters.category.ledomati')}
            </h1>
          </header>
          {products.length === 0 ? (
